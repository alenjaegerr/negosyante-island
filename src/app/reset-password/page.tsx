...
export default function ResetPasswordPage() {
  return (
    <section className="relative mx-auto grid min-h-[calc(100vh-5rem)] w-full max-w-screen-2xl overflow-hidden rounded-3xl border border-[color:var(--ni-border)] bg-[color:var(--ni-surface-1)] shadow-sm lg:grid-cols-[1fr_0.9fr]">
      ...
      <div className="relative order-2 flex items-center overflow-hidden bg-[linear-gradient(135deg,rgba(2,6,23,0.98),rgba(15,23,42,0.95))] p-4 sm:p-8 lg:order-1 lg:p-14">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.24),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(14,165,233,0.16),transparent_38%)]" />
        <div className="relative max-w-2xl space-y-4 text-white sm:space-y-6">
          ...
          <p className="max-w-xl text-[13px] leading-relaxed text-cyan-50/85 sm:text-base">
            Once the code is accepted, your old password stops working immediately.
          </p>
        </div>
      </div>
      ...
      <div className="order-1 flex items-start justify-center bg-[color:var(--ni-surface-1)] p-4 sm:p-8 lg:order-2 lg:items-center lg:p-12">
        <div className="w-full max-w-md space-y-3 sm:space-y-4">
          ...
          <p className="text-center text-[13px] text-[color:var(--ni-text)] sm:text-sm">
            Need a new code? <Link href="/forgot-password" className="font-semibold text-[color:var(--ni-brand)]">Request a reset email.</Link>
          </p>
        </div>
      </div>
    </section>
