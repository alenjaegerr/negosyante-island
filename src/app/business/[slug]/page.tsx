      ...
      <div className="rounded-xl border border-[color:var(--ni-border)] bg-[var(--ni-surface-1)] p-5 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-flex-bold text-2xl text-[var(--ni-text-strong)] sm:text-3xl">{business.name}</h1>
              <p className="mt-1 text-sm text-[var(--ni-text)]">{business.category} • {business.location}</p>
              <p className="mt-1 text-sm font-semibold text-[var(--ni-text-strong)]">
                {business.verified ? "Verified Business ✅" : "Unverified Business"}
              </p>
              <p className="mt-1 text-sm font-semibold text-[var(--ni-text-strong)]">{business.online ? "Online now" : "Offline"}</p>
              <p className="mt-1 text-sm text-[var(--ni-text)]">{business.followers.toLocaleString()} followers</p>
            </div>
          </div>
        ...
        <div className="mt-5 rounded border border-[color:var(--ni-border)] bg-[var(--ni-surface-2)] p-3">
          <h2 className="font-reddit text-xs font-extrabold tracking-figma-tight text-[var(--ni-text-strong)]">ABOUT</h2>
          <p className="mt-2 text-sm text-[var(--ni-text)]">{business.tagline}</p>
        </div>
